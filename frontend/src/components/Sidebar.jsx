...
const Sidebar = ({
    ...
    return (
        <>
            ...
            <aside
                className={`
                    w-80 h-screen fixed left-0 top-0 overflow-y-auto z-50 font-sans glass-dark text-cream flex flex-col shadow-2xl border-right border-white/5 transition-transform duration-300 ease-in-out
                    ${isOpen ? 'translate-x-0' : '-translate-x-full md:translate-x-0'}
                `}
            >
                {/* Header */}
                <div className="p-10 pb-8 border-b border-white/5 flex justify-between items-start">
                    <div>
                        ...
                        <div className="space-y-5" ref={searchRef}>
                            <div className="relative group">
                                <select
                                    ...
                                    onChange={(e) => {
                                        if (e.target.value) handleSelectTicker(tickerOptions[e.target.value]);
                                    }}
                ...
                <div className="p-6 border-t border-white/10 bg-navy-dark/50 backdrop-blur-md">
                    <button
                        ...
                        disabled={loading}
                        className={`
                            w-full py-4 px-6 rounded-lg font-bold tracking-widest uppercase text-xs shadow-lg transition-all duration-300 relative overflow-hidden
                            ${loading
                                ? 'bg-slate-700 text-slate-400 cursor-not-allowed animate-pulse'
                                : 'bg-gradient-to-r from-gold to-yellow-600 text-navy-dark hover:shadow-gold-glow hover:scale-[1.02] active:scale-[0.98]'
                            }
                        `}
                    >
                        {loading && (
